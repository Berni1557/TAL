...
def resample_data_or_seg_to_shape(data: Union[torch.Tensor, np.ndarray],
    ...
    data_reshaped = resample_data_or_seg(data, new_shape, is_seg, axis, order, do_separate_z, order_z=order_z)
    print('data_reshaped12345', len(data_reshaped.shape))
    return data_reshaped
...
def resample_data_or_seg(data: np.ndarray, new_shape: Union[Tuple[float, ...], List[float], np.ndarray],
    ...
    if np.any(shape != new_shape):
        ...
        #print('do_separate_z123', do_separate_z)
        if do_separate_z:
            ...
            # !!!
            # reshaped = []
            # print('data_shape123', data.shape)
            # for c in range(data.shape[0]):
            #     print('c123', c)
            #     #print('dtype_data123', dtype_data)
            #     #print('resize_fn', resize_fn)
            #     #if resize_fn==resize_segmentation:
            #     #    sys.exit()
            #     reshaped.append(resize_fn(data[c], new_shape, order, **kwargs)[None])
            #     #print('reshaped123', reshaped[0].dtype)
            # reshaped_final_data = np.vstack(reshaped)
            # print('reshaped_final_data123', reshaped_final_data.shape, dtype_data)
            # #import sys
            # #sys.exit('exit123')
            
            reshaped_final_data = np.zeros((data.shape[0], new_shape[0], new_shape[1], new_shape[2]), dtype=dtype_data)
            print('reshaped_final_data123XXX', reshaped_final_data.shape)
            
            for c in range(data.shape[0]):
               # print('c123', c)
                reshaped_final_data[c,:,:,:]=resize_fn(data[c], new_shape, order, **kwargs)[None]
                #print('dtype_data123', dtype_data)
                #print('resize_fn', resize_fn)
                #if resize_fn==resize_segmentation:
                #    sys.exit()
                #reshaped_final_data.append(resize_fn(data[c], new_shape, order, **kwargs)[None])
                #print('reshaped123', reshaped[0].dtype)
            #reshaped_final_data = np.vstack(reshaped)
            #print('reshaped_final_data123', reshaped_final_data.shape, dtype_data)
            #import sys
            ...
            # !!!
            #reshaped_final_data_tmp = reshaped_final_data.astype(dtype_data)
            #print('reshaped_final_data_tmp123', reshaped_final_data.shape, dtype_data)

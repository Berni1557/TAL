...
def convert_predicted_logits_to_segmentation_with_correct_shape(predicted_logits: Union[torch.Tensor, np.ndarray],
    ...
    #print('properties_dict123', properties_dict)
    #print('current_spacing123', current_spacing)
    predicted_logits = configuration_manager.resampling_fn_probabilities(predicted_logits,
                                            properties_dict['shape_after_cropping_and_before_resampling'],
                                            current_spacing,
                                            properties_dict['spacing'])
    #print('predicted_logits1234', predicted_logits.shape)
    # return value of resampling_fn_probabilities can be ndarray or Tensor but that does not matter because
    # apply_inference_nonlin will convert to torch
    print('predicted_probabilities456', predicted_logits.shape)
    ...
    print('predicted_probabilities4567', segmentation.shape)

    # put segmentation in bbox (revert cropping)
    segmentation_reverted_cropping = np.zeros(properties_dict['shape_before_cropping'],
                                              dtype=np.uint8 if len(label_manager.foreground_labels) < 255 else np.uint16)
    #print('segmentation_reverted_cropping123', segmentation_reverted_cropping.shape)
...
def export_prediction_from_logits(predicted_array_or_file: Union[np.ndarray, torch.Tensor], properties_dict: dict,
    ...
    #print('export_prediction_from_logits123', export_prediction_from_logits)
    print('export_prediction_from_logits123')
    ...
    print('export_prediction_from_logits123X')
    ...
    print('output_file_truncated123', output_file_truncated)
...
def resample_and_save(predicted: Union[torch.Tensor, np.ndarray], target_shape: List[int], output_file: str,
    ...
    if isinstance(segmentation, torch.Tensor):
        segmentation = segmentation.cpu().numpy()
    print('output_file123', output_file)

...
class nnUNetPredictor(object):
    ...
    def predict_from_data_iterator(self,
        ...
        with multiprocessing.get_context("spawn").Pool(num_processes_segmentation_export) as export_pool:
            ...
            for preprocessed in data_iterator:
                ...
                # !!! BF
                prediction = self.predict_logits_from_preprocessed_data(data, slice_numbers=None).cpu()
    ...
    def predict_single_npy_array(self, input_image: np.ndarray, image_properties: dict,
        ...
        if self.verbose:
            print('predicting')
        #print('dct123', dct['data'].shape)
        predicted_logits = self.predict_logits_from_preprocessed_data(dct['data'], slice_numbers).cpu()
        #print('predicted_logits123', predicted_logits.shape)
    ...
    def predict_logits_from_preprocessed_data(self, data: torch.Tensor, slice_numbers) -> torch.Tensor:
        """
                ...
                try:
                    for params in self.list_of_parameters:
                        ...
                        if prediction is None:
                            prediction = self.predict_sliding_window_return_logits(data, slice_numbers)
                        else:
                            prediction += self.predict_sliding_window_return_logits(data, slice_numbers)
                            
                        #print('prediction1234', prediction.shape)
                        #sys.exit()
    ...
    def _internal_get_sliding_window_slicers(self, image_size: Tuple[int, ...]):
        #print('image_size1236', image_size)
        slicers = []
    ...
    def _internal_maybe_mirror_and_predict_BF(self, x: torch.Tensor) -> torch.Tensor:
        mirror_axes = self.allowed_mirroring_axes if self.use_mirroring else None
        prediction = self.network(x)
        # 
        #print('pred123')
    
        # grad_params = torch.autograd.grad(loss_ce[0][1], params, create_graph=True)
        # fce=None
    ...
    def predict_sliding_window_return_logits(self, input_image: torch.Tensor, slice_numbers=[]) \
            -> Union[np.ndarray, torch.Tensor]:
        assert isinstance(input_image, torch.Tensor)
        self.network = self.network.to(self.device)
        self.network.eval()
        
        #print('predict_sliding_window_return_logits1234')
        ...
        with torch.no_grad():
            with torch.autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
                ...
                # print('XXX')
                # print('input_image123', input_image.shape)
                # print('pad_nd_image123', pad_nd_image)
                # sys.exit()
                ...
                #import sys
                #sys.exit()
                #slicers_tmp = self._internal_get_sliding_window_slicers(data.shape[1:])
                
                #print('slicers0', slicers_tmp[0])
                #print('slice_numbers123', slice_numbers)
                # print('slicers1', slicers_tmp[1])
                # print('slicers2', slicers_tmp[2])
                # print('slicers3', slicers_tmp[3])
                # print('slicers4', slicers_tmp[4])
                # print('slicers5', slicers_tmp[5])
                # print('slicers6', slicers_tmp[6])
                # print('slicers7', slicers_tmp[7])
                
                #print('slicers_tmp123', len(slicers_tmp))
                # if slice_numbers is not None:
                #     slicers=[]
                #     for sli in slicers_tmp:
                #         if sli[1] in slice_numbers:
                #             slicers.append(sli)
                # else:
                #     slicers = slicers_tmp
                #slicers = slicers_tmp
                    
               # print('slicers123', slicers)
                    
                #slicers = [slicers[10],slicers[200],slicers[300],slicers[400]]
                #print('slicers123', type(slicers))
                #print('slicers123', slicers[20][1])
                #import sys
                #sys.exit()
        ...
        empty_cache(self.device)
        #print('predicted_logits12345', predicted_logits.shape)
        #print('slicer_revert_padding1234', slicer_revert_padding)
        #q=predicted_logits[tuple([slice(None), *slicer_revert_padding[1:]])]
        #print('q123', q.shape)
        #sys.exit()
        return predicted_logits[tuple([slice(None), *slicer_revert_padding[1:]])]

    def predict_sliding_window_return_logits_BF(self, input_image: torch.Tensor, slice_numbers=[]) \
            -> Union[np.ndarray, torch.Tensor]:
        assert isinstance(input_image, torch.Tensor)
        self.network = self.network.to(self.device)
        self.network.eval()
        ...
        with torch.autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
           ...
           # print('slicers123', slicers)
            ...
            for sl in tqdm(slicers, disable=not self.allow_tqdm):
                ...
                prediction = self._internal_maybe_mirror_and_predict_BF(workon)[0].to(results_device)

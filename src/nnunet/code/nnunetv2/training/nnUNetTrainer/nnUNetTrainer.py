...
#from modules.UNetAL.data_loader_2d_BF import nnUNetDataLoader2DBF
#from modules.UNetAL.data_loader_3d_BF import nnUNetDataLoader3DBF
...
class nnUNetTrainer(object):
    ...
    def get_dataloaders_BF(self, data_load, unlabeled=True, batch_size=None):
        # self=net.model['unet']
        ...
        dl_ul = self.get_plain_dataloaders_BF(initial_patch_size, dim, data_load, unlabeled, batch_size)
        #dl_tr.infinite=True
    ...
    def get_plain_dataloaders_BF(self, initial_patch_size: Tuple[int, ...], dim: int, data_load: list, unlabeled: bool, batch_size: int):
        #dataset_tr, dataset_val = self.get_tr_and_val_datasets()
        ...
        if dim == 2:
            ...
            dl_ul = nnUNetDataLoader2DBF(dataset_ul, batch_size,
                                        self.configuration_manager.patch_size,
                                        self.configuration_manager.patch_size,
                                        self.label_manager,
                                        oversample_foreground_percent=self.oversample_foreground_percent,
            ...
            #                            infinite=False,
            #                            sampling_probabilities=None, pad_sides=None)
            dl_ul = nnUNetDataLoader3DBF(dataset_ul, self.batch_size,
                                       self.configuration_manager.patch_size,
                                       self.configuration_manager.patch_size,
                                       self.label_manager,
                                       oversample_foreground_percent=self.oversample_foreground_percent,
    ...
    def get_training_transforms(patch_size: Union[np.ndarray, Tuple[int]],
        ...
        #print('patch_size_spatial123', patch_size_spatial)
        #sys.exit()
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            #!!!
            #print('target123', torch.unique(target[0]))
            # del data
        ...
        # print('output123', output[0].shape)
        # im = data[0,0,:,:].detach().cpu().numpy()
        # tar = target[0][0,0,:,:].detach().cpu().numpy()
        # #out = output[0][0,0,:,:].detach().cpu().numpy().astype(np.double)
        # out = output[0][0,1:,:,:].detach().cpu().numpy().sum(axis=0).astype(np.double)
        # if np.any(tar>0):
        #     print('out123', out[200:300,200:300])
        #     plt.imshow(im)
        #     plt.show()
        #     plt.imshow(tar)
        #     plt.show()
    ...
    def run_training(self):
        ...
        for epoch in range(self.current_epoch, self.num_epochs):
            ...
            for batch_id in range(self.num_iterations_per_epoch):
                ...
                qtmp = next(self.dataloader_train)
                #print('qtmp123', qtmp['data'].shape)
                train_outputs.append(self.train_step(qtmp))

...
class ALAction():
  ...
  def serialize(self):
    ...
    d['slice'] = self.slice
    if self.bboxLbsOrg is not None:
        d['bboxLbsOrg'] = [int(x) for x in list(self.bboxLbsOrg)]
    if self.bboxLbsOrgW is not None:
        d['bboxLbsOrgW'] = [int(x) for x in list(self.bboxLbsOrgW)]
    if self.bboxUbsOrg is not None:
        d['bboxUbsOrg'] = [float(x) for x in list(self.bboxUbsOrg)]
    if self.bboxUbsOrgW is not None:
  ...
  def load(fip_actionlist):
    ...
    Load actionlist
    """
    if os.path.isfile(fip_actionlist):

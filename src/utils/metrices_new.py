...
def cldice(target, pred, num_classes=-1, binary_class=-1, mode='binary', class_names=None):
    #if num_classes==-1:
    #    num_classes = num_classes = C.shape[0]
    ...
    def clDice(v_p, v_l):
        ...
        if (tprec+tsens)==0:
            ...
            di = 2*tprec*tsens/(tprec+tsens) 
         
        return di, tprec, tsens
        ...
        if binary_class==-1:
            ...
            for c in range(num_classes):
                ...
                if t.sum()==0 and p.sum()==0:
                    ...
                    dice_tub = clDice(p, t)
                    dice.append(dice_tub[0])
                    prec.append(dice_tub[1])
                    sens.append(dice_tub[2])
                
            if class_names is not None:
                dice = {class_names[i]: dice[i] for i in range(len(class_names))}
                prec = {class_names[i]: prec[i] for i in range(len(class_names))}
                sens = {class_names[i]: sens[i] for i in range(len(class_names))}
            else:
                dice = {str(i): dice[i] for i in range(num_classes)}
                prec = {str(i): prec[i] for i in range(num_classes)}
                sens = {str(i): sens[i] for i in range(num_classes)}
        elif binary_class is None: 
            t = target>0
            p = pred>0
            dice_tub = clDice(p, t)
